...
const postdata = {
  ...
  questions: [
    ...
    "To Enable AVIF Support in Mozilla Firefox,",
    "Enable AVIF Support in Mozilla Firefox",
...
export default function BlogAvifInEdge() {
  return (
    <Blog postdata={postdata} posts={posts}>
      ...
      Edge, you should download the latest version. The new Microsoft Edge is based on Chromium and
      was first released first at the beginning of 2020. It is compatible with all versions of
      Windows and macOS (for whatever reason). Microsoft claims it's the only browser you will ever
      need, but well, who wouldn't say that about their own browser. You may quickly determine if
      you own a Chromium version of Edge by looking at the browser icon. The new version of Edge has
      a more minimalistic and gradient style, where the old one looks like a Van Gogh.
      <BlogSubtitle text="AV1 Plugin" />
      The AV1 Extension add-on is available from the Microsoft Store. It provides support for AV1
      videos on Windows 10 devices and Edge. It also allows for viewing avif images, including
      thumbnails, and editing them in Paint. Furthermore, this extension allows video apps installed
      on Windows 10 to play videos encoded using the AV1 video coding standard developed by the
      Alliance for Open Media. As already stated, there isn't support for AVIF images yet, even
      though avif.io converts images seamlessly on Edge. However, since Microsoft Edge uses the same
      Chromium Engine, the support for Edge should roll out soon.
    </Blog>
  );
}
